...
export function runOpenCode({ connection, prompt, workspace, effort, isoBase, timeoutMs = 240000, approvedRoots,
  ...
  // `task` (skills/agentSlot directives) and `locale` are server-path-only additions: skill/agent
  // routing onto the live OpenCode registry + context-compaction planner-defect signal. Both are
  // no-ops on the CLI path below — passing them when the flag is unset changes nothing.
  if (process.env.BO_CE_OPENCODE_SERVER === "1") {
    return import("./opencode-server.mjs").then(({ runOpenCodeServer }) =>
      runOpenCodeServer({ connection, prompt, workspace, effort, isoBase, timeoutMs, approvedRoots, task, locale }));
  }
