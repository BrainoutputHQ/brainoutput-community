...
async function runWorkerStage({ node, wprompt, codeWs, index, maxTokens = 1500, timeoutMs = 240000, onSessionStart = () => {} }) {
...
async function runSpineTaskWorker({ task, ctx, maxTokens = 1500, timeoutMs = 240000 }) {
  ...
  for (;;) {
    ...
    const out = await runWorkerStage({ node: ctx.workerNode, wprompt, codeWs: ctx.codeWs,
      index: ctx.index, maxTokens, timeoutMs, onSessionStart: ctx.onSessionStart || (() => {}) });
